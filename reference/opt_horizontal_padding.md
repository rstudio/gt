...
Increase or decrease the horizontal padding throughout all locations of
a **gt** table by use of a `scale` factor, which here is defined by a
real number between `0` and `3`. This function serves as a shortcut for
...
- scale:
  ...
  *Scale factor*
  ...
  A scale factor by which the horizontal padding will be adjusted. Must
...
and more). Following that, we'll increase the amount of horizontal
padding across the entire table with `opt_horizontal_padding()`. Using a
`scale` value of `3` (up from the default of `1`) means the horizontal
space will be greatly increased, resulting in a more spacious table.
...
\`opt_horizontal_padding()\` help
file.](https://raw.githubusercontent.com/rstudio/gt/master/images/man_opt_horizontal_padding_1.png)
