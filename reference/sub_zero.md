...
Wherever there is numerical data that are zero in value, replacement
text may be better for explanatory purposes. `sub_zero()` allows for
this replacement through its `zero_text` argument.
...
- zero_text:
  ...
  *Replacement text for zero values*
  ...
  The text to be used in place of zero values in the rendered table. We
...
With this table, the zero values in will be given replacement text with
a single call of `sub_zero()`.
...
\`sub_zero()\` help
file.](https://raw.githubusercontent.com/rstudio/gt/master/images/man_sub_zero_1.png)
