...
Add summary rows to one or more row groups by using the table data and
any suitable aggregation functions. Multiple summary rows can be added
for selected groups via expressions given to `fns`. You can selectively
format the values in the resulting summary cells by use of formatting
expressions in `fmt`.
...
- side:
  ...
  *Side used for placement of summary rows*

  `singl-kw:[bottom|top]` // *default:* `"bottom"`

  Should the summary rows be placed at the `"bottom"` (the default) or
  the `"top"` of the row group?
...
groups) to `fmt`, and, several can be provided in a list. The RHS will
always contain an expression that uses a formatting function (e.g.,
[`fmt_number()`](https://gt.rstudio.com/reference/fmt_number.md),
[`fmt_currency()`](https://gt.rstudio.com/reference/fmt_currency.md),
etc.) and it must contain an initial `.` that stands for the data
object. If performing numeric formatting on all columns in the new
...
A two-sided expression is needed for targeting the formatting directives
to specific summary row groups. In this format, the LHS should contain
an expression that resolves to a set of available groups. We can use a
single row group name in quotes, several of those in a vector, or a
select helper expression like
[`starts_with()`](https://tidyselect.r-lib.org/reference/starts_with.html)
or
[`matches()`](https://tidyselect.r-lib.org/reference/starts_with.html).

In a situation where summary rows were generated across the row groups
named `"group_1"`, `"group_2"`, and `"group_3"`, we could format all
summary cells in `"group_2"` with the following:
...
targeting could be paired with single values for `columns` and `rows` on
the RHS formatting expression. Like this:
...
**gt** table with row groups and row labels. Create the summary rows
labeled `min`, `max`, and `avg` by row group (where each each row group
is a week number) with `summary_rows()`.
...
\`summary_rows()\` help
file.](https://raw.githubusercontent.com/rstudio/gt/master/images/man_summary_rows_1.png)

Using the
[`countrypops`](https://gt.rstudio.com/reference/countrypops.md)
dataset, let's process that a bit before giving it to **gt**. We can
create a summary rows with totals that appear at the top of each row
group (with `side = "top"`). We can define the aggregation with a list
that contains parameters for the summary row label (`md("**ALL**")`),
the shared ID value of those rows across groups (`"totals"`), and the
aggregation function (expressed as `"sum"`, which **gt** recognizes as
the [`sum()`](https://rdrr.io/r/base/sum.html) function). To top it all
off, we'll add background fills to the summary rows with
...
the \`summary_rows()\` help
file.](https://raw.githubusercontent.com/rstudio/gt/master/images/man_summary_rows_2.png)
