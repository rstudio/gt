...
Wherever there is missing data (i.e., `NA` values) customizable content
may present better than the standard `NA` text that would otherwise
appear. `sub_missing()` allows for this replacement through its
...
- missing_text:
  ...
  *Replacement text for `NA` values*
  ...
  The text to be used in place of `NA` values in the rendered table. We
  can optionally use [`md()`](https://gt.rstudio.com/reference/md.md) or
...
create a **gt** table. The `NA` values in different columns (here, we
are using column indices in `columns`) will be given two variations of
...
\`sub_missing()\` help
file.](https://raw.githubusercontent.com/rstudio/gt/master/images/man_sub_missing_1.png)
