...
Format values in a vector to datetime values using either presets for
the date and time components or a formatting directive (this can either
use a *CLDR* datetime pattern or `strptime` formatting). Input can be in
the form of `POSIXct` (i.e., datetimes), the `Date` type, or `character`
...
options (here, `date_style = "yMMMEd"` and `time_style = "Hm"`) will
result in a character vector of formatted datetime values. Any `NA`
...
We can choose from any of 41 different date styles and 25 time
formatting styles. Many of these styles are flexible, meaning that the
structure of the format will adapt to different locales. Let's use a
combination of the `"yMMMd"` and `"hms"` date and time styles to
...
or
[`info_time_style()`](https://gt.rstudio.com/reference/info_time_style.md)
to call up info tables that serve as handy references to all of the
`date_style` and `time_style` options.

It's possible to supply our own time formatting pattern within the
`format` argument. One way is with a CLDR pattern, which is
locale-aware:
...
It's also possible to use a `strptime` format code with `format`
(however, any value provided to `locale` will be ignored).
...
As a last example, one can wrap the datetime values in a pattern with
the `pattern` argument. Note here that `NA` values won't have the
pattern applied.
