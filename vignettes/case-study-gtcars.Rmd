...
It's sometimes useful to arrange variables/columns into groups by using spanner headings. This can be done in **gt** by using the `tab_spanner()` function. It takes the `label` and `columns` arguments; the `label` is the spanner column label and the `columns` are those columns that belong in this group.
