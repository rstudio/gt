...
Let's make a display table using the `gtcars`. We all know `mtcars`... what is `gtcars`? It's basically a modernized `mtcars` for the **gt** age. It's part of the **gt** package, and here is a preview of the tibble:

```{r gtcars_data_frame}
# This is `gtcars`
dplyr::glimpse(gt::gtcars)
```
...
Let's make a display table from this dataset. In doing so we'll fulfil the following 10 requirements:
...
It's sometimes useful to arrange variables/columns into groups by using spanner column labels. This can be done in **gt** by using the `tab_spanner()` function. It takes the `label` and `columns` arguments; the `label` is the spanner column label and the `columns` are those columns that belong in this group.
...
```{r tab_spanner}
# Put the first three columns under a spanner column with
...
Sometimes we'd like to combine the data from two columns into a single column while describing the way the data is to be combined. The `cols_merge()` function allows for this. For our table, let's merge together the following pairs of columns:
...
The `cols_merge()` uses a `col_1` column and a `col_2` column. Once combined, the `col_1` column will be retained and the `col_2` column will be dropped. The pattern argument uses `{1}` and `{2}` to represent the content of `col_1` and `col_2`. Here, we can use string literals to add text like `rpm` or the `@` sign. Furthermore, because we are targeting an HTML table, we can use the `<br>` tag to insert a linebreaks.

Labeling columns essentially means that we are choosing display-friendly labels that are no longer simply the column name (the default label). The `cols_label()` function makes this relabeling possible. It accepts a series of named arguments in the form of `<column_name> = <column_label>, ...`.
...
```{r tab_header}
# Add a table title and subtitle; we can use Markdown
...
What `cells_data()` expects is `columns` (column names, which can be conveniently provided in `vars()`) and `rows` (which can be a vector of row names or row indices). The `cells_stub()` location helper only expects a vector of `rows`. For `cells_columns()`, we can either provided targeted column labels in the `columns` argument or spanner column labels in the `groups` argument. Here, we are targeting a footnote to the `msrp` column label so we will use `columns = vars(msrp)`.
