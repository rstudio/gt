...
We can think of display tables as output only, where we'd not want to use them as input ever again. Other feature include annotations, table element styling, and text transformations that serve to communicate the subject matter more clearly.
...
The main entry point into the **gt** API is the `gt()` function. If we pass `islands_tbl` to the *function* `gt()`, we'll get a display table as output. As a side note, we could have easily generated a data frame instead as perfectly valid input data for **gt**.
...
That doesn't look too bad. Sure, it's basic but we really didn't really ask for much. We did receive a proper table *boxhead* (with column labels!) and a *field*. Also, that default striping is a nice touch. Oftentimes however, you'll want a bit more: a *title*, a *stub*, *source notes*, and sometimes *footnotes*.

### Adding Parts to this Simple Table

The package makes it relatively easy to add parts so that the resulting display table. These table parts work well together and there is logic behind about how the information held within the parts is arranged. The last display table shown had only two parts, the **boxhead** and the **field**, the next few display tables will have considerably more parts.

The way the main parts of a table (and their subparts) fit together is best shown as a schematic:

<p align="center"><img src="../man/figures/gt_parts_of_a_table.png" width=100%></p>
...
The way that we add parts like the **heading** and **footnotes** is to use the `tab_*()` family of functions. A heading is easy to add so let's see how the previous table looks with a **title** and a **headnote**. We can add this part using the `tab_heading()` function:
...
A very important thing to note here is that the table now has one **field** column. Before, when there was no **stub**, two columns were present (with **column labels** `name` and `size`) but now column `1` (the only **field** column) is `size`. This has implications for column referencing (for instance, with the `cells_data()` helper function). Before, we referenced names in data cells using `where`. Now, the names are row captions and they can be referenced in the `rows` argument. To apply our table parts as before (up to and including the footnotes) we use the following statements:
...
<img src="images/gt_stub_block_boxhead_panel.png" width=100%>
...
Note that even though columns were moved using `cols_move_to_start()`, the **spanner headings** still spanned above the correct columns. There are a number of functions that **gt** provides to move columns, including `cols_move()`, `cols_move_to_end()`; there's even a function to hide columns: `cols_hide()`.
