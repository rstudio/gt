...
```{r}
# Generate a gt table after creating a `rowname` colummn
islands_tbl %>%
  rename(rowname = name) %>%
  gt()
```
...
A very important thing to note here is that the table now has one **field** column. Before, when there was no **stub**, two columns were present (with **column labels** `name` and `size`) but now column `1` is `size`. This has implications for column referencing (for instance, with the `data_cells()` helper function). To apply our table parts as before, we use the following statements:
...
Let's take this further and add one more magic column to `islands_tbl` as a way to incorporate **stub blocks** within the **stub**. These blocks can be considered groups of rows and so it's very logical that the magic column name is `groupname`. A `groupname` column will be added to `islands_tbl` and it will consist of categories that describe the landmass type.

```{r}
# Display the `islands_tbl` data with a stub,
# a heading, source notes, and footnotes; group heading
# rows are added through the `groupname` column
islands_tbl %>%
  rename(rowname = name) %>%
  mutate(groupname = c(
