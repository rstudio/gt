...
The `islands_tbl` tibble is now suitable input for **gt**. As a side note, we could have easily generated a data frame instead as perfectly valid input data for **gt**. If we pass that table to the *function* `gt()`, we'll get a display table as output.
