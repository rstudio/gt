...
# table; add a header with a title and subtitle
gt_tbl <- 
  gt_tbl %>%
  tab_header(
...
The **Header** provides a nice opportunity to describe the data that's presented. The *subtitle* is an optional part of the **Header**. We might also want to style the *title* and *subtitle* using markdown! We do this by wrapping the values passed to `title` or `subtitle` with the `md()` function. Here is an example with the **Table Data** truncated for sake of brevity:
...
An easy way to generate a **Stub** part is by having an input dataset with a column named `rowname`. This magic column will signal to **gt** that the data in that column should be placed in the stub as *row labels*. (An alternative method is to specify a column name in the `rowname_col` argument of the `gt()` function.) To make this work with our `islands_tbl` dataset, we'll just change rename the column `name` to `rowname`, and then use `gt()` to view the table w/ stub:
...
Notice that the landmass names are off the the left in an unstriped area? That's the **Stub**. We can apply what's known as a *stubhead label*, which is label text that resides in the area above the **Stub** and left of the **Column Labels**. This operation is done with the `tab_stubhead_label()` function:
