...
adsl_summary <- purrr::map_df(
      ...
      .f = \(x) custom_summary(df = dplyr::filter(rx_adsl, ITTFL == "Y"), group_var = TRTA, sum_var = !!x)
    )
```
...
This is the dataset that serves as a starting point for `gt`. We will start by exposing the dataset to `gt` and add our usual headers and footers.
