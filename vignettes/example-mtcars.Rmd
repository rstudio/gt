...
We can choose which columns will have summary data computed (here it makes less sense to include `cyl`) in the optional `columns` argument. The `fns` argument can take one or more aggregation functions, preferably within a `c()` or `list()` statement. In the example below, we are providing the names of functions in quotes. This is great if we want to use the default parameters in the given functions. Alternatively, we can specify functions as R formulas (e.g., `~mean(., na.rm = TRUE)`) where the `.` stands in for the data to be aggregated. If we use named arguments within the `c()` or `list()`, they will serve as summary caption labels (otherwise **gt** will generate names based on the functions). Finally, we can specify a *formatter* and that formatter's arguments/values so that we have a chance to format the output summary values in the way we prefer them to look.
