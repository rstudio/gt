...
#' a table with `local_image()` function. The function provides a
...
#' that the [text_transform()] function is used. With that function,
#' we can specify which data cells to target and then include a
#' `local_image()` call within the required user-defined function (for the
#' `fn` argument). If we want to include an image in other places (e.g., in
...
#' available in the \pkg{gt} package using the [test_image()]
...
#' is recommended that the [text_transform()] function is used. With
#' that function, we can specify which data cells to target and then include a
#' `web_image()` call within the required user-defined function (for the
#' `fn` argument). If we want to include an image in other places (e.g., in
...
#' [text_transform()] function is used. With that function, we can
#' specify which data cells to target and then include a call to
#' `ggplot_image()` within the required user-defined function (for the
#' `fn` argument). If we want to include a plot in other places (e.g., in
...
#' [local_image()] since we test various sizes of the test image
#' within that function.
#' @param type the type of the image, which can either be `png` (the
#'   default) or `svg`.
#' @family image addition functions
#' @export
