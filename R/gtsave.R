...
#' \pkg{webshot2} package, so, this package needs to be installed before
...
#' available so have a look at the \pkg{webshot2} documentation for further
#' details.
#'
...
gtsave <- function(data,
  ...
  switch(file_ext,
         ...
         "html" = gt_save_html(data, filename, path, ...),
          ...
          "png" = ,
          "pdf" = gt_save_webshot2(data, filename, path, ...),
         {
...
#'
#' @noRd
gt_save_webshot2 <- function(data,
                             filename,
                             path = NULL,
                             ...,
                             zoom = 2,
  ...
  # Reverse slashes on the Windows filesystem
  tempfile_ <-
    tempfile_ %>%
    tidy_gsub("\\\\", "/")
