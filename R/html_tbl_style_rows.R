...
add_style_to_summary_caption_rows <- function(html_tbl,
  ...
  # Determine which rows are those with a group heading
  summary_caption_rows <-
    html_tbl[["html_table"]] %>%
    dplyr::filter(t_subpart == "summary_caption")

  # Determine which rows will have the styles applied
  if (nrow(summary_caption_rows) > 0) {
  ...
  # Determine which columns will have the styles applied
  if (extend_to_field == FALSE) {
  ...
  if (!(property %in% colnames(html_tbl[["html_table"]]))) {
    ...
    html_tbl[["html_table"]] <-
      dplyr::bind_rows(
        html_tbl[["html_table"]] %>%
          dplyr::filter(row %in% summary_caption_rows & column %in% apply_to_columns) %>%
          dplyr::mutate(rlang::UQ(property) := values),
        html_tbl[["html_table"]] %>%
          dplyr::filter(!(row %in% summary_caption_rows & column %in% apply_to_columns)) %>%
          dplyr::mutate(rlang::UQ(property) := NA_character_)) %>%
...
add_style_to_group_heading_rows <- function(html_tbl,
  ...
  group_heading_rows <-
    html_tbl[["html_table"]] %>%
    dplyr::filter(t_subpart == "group_heading")

  # Determine which rows will have the styles applied
  if (nrow(group_heading_rows) > 0) {
  ...
  if (!(property %in% colnames(html_tbl[["html_table"]]))) {
    ...
    html_tbl[["html_table"]] <-
      dplyr::bind_rows(
        html_tbl[["html_table"]] %>%
          dplyr::filter(row %in% group_heading_rows & column %in% apply_to_columns) %>%
          dplyr::mutate(rlang::UQ(property) := values),
        html_tbl[["html_table"]] %>%
          dplyr::filter(!(row %in% group_heading_rows & column %in% apply_to_columns)) %>%
          dplyr::mutate(rlang::UQ(property) := NA_character_)) %>%
