...
#'
#' The `fmt_time()` function lets us format time-based values in a
#' convenient manner using preset styles. The table generated by the
#' `info_time_style()` function provides a quick reference to all five
#' styles, with associated number codes, the format names, and example outputs
...
#'
#' The `fmt_currency()` function lets us format numeric values as
#' currencies. The table generated by the `info_currencies()` function
...
#' `49.95`) to demonstrate the default formatting of each currency.
#'
...
#'   `code` where currency information corresponding to 3-letter currency
#'   codes is provided, or `symbol` where currency info for common currency
...
#'   (`NULL`) will produce a table with all currencies displayed. This
#'   option only constrains the information table where `type == "code"`.
#' @examples
...
#' common display name) along with the each locale's group and decimal separator
#' marks. A formatted numeric example is provided (based on the value of
#' `11027`) to demonstrate the default formatting of each locale.
#'
...
#' letter corresponding to the base locale ID to `begins_with`. This will
#' filter locales in the info table to just the set beginning with the supplied
#' letter.
#'
