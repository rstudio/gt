...
#' use `gt_preview()` in place of [gt()] to get the first x
#' rows of data and the last y rows of data (which can be set by the
#' `top_n` and `bottom_n` arguments). It's not advised to use
#' additional \pkg{gt} API functions to further modify the output of
#' `gt_preview()`. Furthermore, you cannot pass a \pkg{gt} object to
#' `gt_preview()`.
#'
#' Any grouped data or magic columns such as `rowname` and `groupname`
#' will be ignored by `gt_preview()` and, as such, one cannot add a stub or
...
#'   of the table to display. The default, `1`, will show the final row of
#'   the table.
#' @param incl_rownums an option to include the row numbers for `data` in
#'   the table stub. By default, this is `TRUE`.
