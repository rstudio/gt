...
gt_save_webshot <- function(
  ...
  # Edit the document to inject styles in the head
  style_old <- "<style>body{background-color:white;}</style>"
  ...
  document <- chr_sess$DOM$getDocument()
  table_node <- chr_sess$DOM$querySelector(document$root$nodeId, ".gt_table")
  box_model <- chr_sess$DOM$getBoxModel(nodeId = table_node$nodeId)

  width <- box_model$model$width + 3
  height <- box_model$model$height - box_model$model$margin[[7]]
  ...
  html_table <- readLines(con = tempfile_)
  html_table <- gsub(style_old, style_new, html_table, fixed = TRUE)

  writeLines(text = html_table, con = tempfile_)
