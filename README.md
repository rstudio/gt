...
`tab_create()` function to initiate the process (providing `mtcars` to
the `tbl` argument), creating a table object. Because `mtcars` is a data
frame with rownames, a stub is automatically created: it contains the
row names that are really the car models. After that, a few functions
can be used to refine the display table:
...
# 7 - a heading is added, both parts are interpreted as Markdown
# 8 - two source notes are added, the first is interpreted as Markdown
mtcars_tbl <-
...
iris_tbl <-
  ...
  tab_heading(
    title = md("Excerpt from the **iris** dataset"),
    headnote = md("[All about *Iris setosa*, *versicolor*, and *virginica*]")
    ) %>%  # 6
  tab_source_note(
    source_note = md("The data were collected by *Anderson, Edgar* (1935).")
    )  # 7
```
