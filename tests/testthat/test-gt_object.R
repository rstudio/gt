...
test_that("an HTML table object contains the correct components", {
  ...
  # Expect that the list object has 11 components
  expect_equal(
    length(table_object), 11)

  # Expect certain named components
  expect_equal(
    table_object %>% names(),
    c("transform_directives", "aesthetics",
      "source_tbl", "modified_tbl",
      "heading", "footnote", "source_note", "stubhead_caption",
      "boxhead_panel",
