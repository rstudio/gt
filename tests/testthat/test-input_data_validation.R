...
test_that("all exported functions validate the incoming `data` object", {
  ...
  expect_error(exibble %>% opt_add_row_striping(), regexp = regexp_stop)
  expect_error(exibble %>% opt_align_table_header(), regexp = regexp_stop)
  expect_error(exibble %>% opt_all_caps(), regexp = regexp_stop)
  expect_error(exibble %>% opt_remove_top_bottom_borders(), regexp = regexp_stop)
  expect_error(exibble %>% opt_fully_lined(), regexp = regexp_stop)
  expect_error(exibble %>% opt_lineless(), regexp = regexp_stop)
  expect_error(exibble %>% opt_add_table_outline(), regexp = regexp_stop)
  expect_error(exibble %>% opt_remove_table_outline(), regexp = regexp_stop)
