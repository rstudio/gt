...
test_that("Classes and styles are parsed correctly", {
  ...
  expect_equal(
    style[[1]]$text_gp,
    grid::gpar(fontfamily = "Times New Roman", fontsize = 18.0675,
               col = "#00FF00", fontface = "italic")
  )

  expect_equal(
    style[[1]]$border_gp,
    grid::gpar(lwd = c(2.26, 1.13, 1.13),
               lty = c(3, 1, 1),
  ...
  expect_equal(
    ...
    list(vjust = 0.5, hjust = 1, margin = grid::unit(c(6, 3.7, 6, 3.7), "pt"),
