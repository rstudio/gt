...
mtcars_tbl <-
  ...
  tab_footnote(
    ...
    location = target_cell(row = "Ford Pantera L", column = vars(qsec))) %>% # 11
  tab_footnote(
    footnote = "Massive hp.",
    location = target_cell(row = "Ford Pantera L", column = vars(hp))) %>% # 11
  tab_footnote(
    footnote = "Excellent gas mileage.",
    location = target_cell("Toyota Corolla", column = 2)) %>% # 11
  tab_footnote(
    footnote = md("Worst speed *ever*."),
    location = target_cell("Merc 230", vars(qsec))) %>% # 11
  cols_label(
    labels = col_labels(
