...
[![Travis build status](https://travis-ci.org/rstudio/gt.svg?branch=master)](https://travis-ci.org/rstudio/gt)
...
```{r echo=FALSE}
table_html <-
  gt(mtcars[1:6, ], rownames_to_stub = TRUE) %>% 
  fmt_number(
    columns = vars(mpg, disp, drat, qsec),
    decimals = 1
    ) %>%
  ...
  tab_stubhead_caption(caption = "car model") %>%
  tab_heading(
    title = md("Excerpt from the **mtcars** dataset"),
    headnote = md("[A rather famous *Motor Trend* table]")
    ) %>%
