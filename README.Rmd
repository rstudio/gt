...
The HTML table object can then be transformed to HTML with the `emit_html()` function. This works well inside an R Markdown code chunk since the table will appear when the document is rendered to HTML.

```r
mtcars_tbl %>% emit_html()
```

We can also preview the table in the RStudio Viewer pane (or the default browser if invoked outside of RStudio) with `render_table()`:
